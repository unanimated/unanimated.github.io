...
function important(subs, sel)
	...
	if res.mega=="import sign" then
		...
		{x=0,y=4,width=3,height=1,class="checkbox",name="noshift",label="don't shift times",value=false,},
		},{"OK"},{ok='OK'})
	...
	if res.mega:match("import") and x==1 then
	    ...
	    if reslt.noshift then shiftime=0 end
	    ...
	    text=btext line.comment=true line.end_time=basend line.style=basestyle line.actor="" line.effect="" end
	end
	...
	if res.mega=="update lyrics" then
		...
		if marker:match(sub1.."%d+"..sub2) and denumber==sub1..sub2 and pass==1 then
		    ...
		    songcheck=1
		    if songlyr[index]~=puretext then change="   (Updated)" else change="" end
