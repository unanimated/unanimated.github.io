...
function fadalpha(subs, sel)
	for z, i in ipairs(sel) do
		...
		primary=styleref.color1		primary=primary:gsub("H%x%x","H")
		pri=text:match("^{\\[^}]-\\c(&H%x+&)")		if pri~=nil then primary=pri end
		outline=styleref.color3		outline=outline:gsub("H%x%x","H")
	...
	-- with alpha in line
		...
		-- fade to colour
			    if res.clr then
			text=text:gsub("^({\\[^}]-)}","%1\\t(" ..duration-fadout..",0," ..res.ut.. ","..kolorb..")}")
		-- inline colour tags
			for t in text:gmatch("({\\[^}]-})") do
				if t~=text:match("^{\\[^}]-}") and t:match("\\[13]?c") then
			t2=t:gsub("({\\[^}]-)}","%1\\t(" ..duration-fadout..",0," ..res.ut.. ","..kolorb..")}")
		...
		-- fade to alpha
			...
			text=text:gsub("^({\\[^}]-)}","%1\\t(" ..duration-fadout..",0," ..res.ut.. ","..aff..")}")
		-- inline alpha tags
			for t in text:gmatch("({\\[^}]-})") do
				if t~=text:match("^{\\[^}]-}") and t:match("\\alpha") then
				
			t2=t:gsub("({\\[^}]-)}","%1\\t(" ..duration-fadout..",0," ..res.ut.. ","..aff..")}")
			t=esc(t)
			text=text:gsub(t,t2)
				end
			end
