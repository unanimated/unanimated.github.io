...
tdura=0		tlength=0	tcps=0		trilin=0	par=0
  ...
  if pressed==">QC" then
	...
	if res["malformed"] then
	  ...
	  for i=1,#tugs1 do tag=tugs1[i]
		...
		if not tak:match("^%-?%d[%d%.]-$") and tak~="" then effect=effect.." [malformed tags1]" malf=malf+1 end
		end
	    end
	  end
	  for i=1,#tugs2c do tag=tugs2c[i]
	    if text:match("\\"..tag) then
		for tak in text:gmatch("\\"..tag.."([^\\}%)]-)[\\}%)]") do
		if not tak:match("^&H%x%x%x%x%x%x&$") and tak~="" then effect=effect.." [malformed tags2]" malf=malf+1 end
		end
	    end
	  end
	    if text:match("\\c&") then
		for tak in text:gmatch("\\c([^\\}%)]-)[\\}%)]") do
	  if not tak:match("^&H%x%x%x%x%x%x&$") and not tak:match("lip") and tak~="" then effect=effect.." [malformed tags2]" malf=malf+1 end
		end
	    end
	  for i=1,#tugs2a do tag=tugs2a[i]
	    if text:match("\\"..tag) then
		for tak in text:gmatch("\\"..tag.."([^\\}%)]-)[\\}%)]") do
		if not tak:match("^&H%x%x&$") and tak~="" then effect=effect.." [malformed tags2]" malf=malf+1 end
		end
	    end
	  end
	  for i=1,#tugs3 do tag=tugs3[i]
	    if text:match("\\"..tag) then
		for tak in text:gmatch("\\"..tag.."([^\\}]-)[\\}]") do
		if not tak:match("^%([%d%.,%-]-%)$") then effect=effect.." [malformed tags3]" malf=malf+1 end
		end
	    end
	  end
	if parenth~="" then effect=effect.." [parentheses fail]" par=par+1 end
	end
	...
	if res["overlap"] and not actor=="qcd" then
	...
	if visible:match(" the my ") or visible:match(" the h[ei][rs] ") then effect=effect.." the what?" end
	...
	-- faggosaurosis count
	if res.sauro then
	  saureff=effect:gsub(" %[\"%]","")  lsaureff=line.effect:gsub(" %[\"%]","")
    ...
    if malf~=0 then report=report.."Malformed tags found... "..malf.."\n" end
    ...
    if sorted==0 and mblur==0 and malf==0 and dis==0 and par==0 and over==0 and gap==0 and dspace==0 and dotdot==0 and badita==0 and comfail==0 and unreadable==0 and misstyles=="" and colorspace~="TV.601" then
...
function konfig(subs, sel)
	...
	{x=1,y=10,width=1,height=1,class="checkbox",name="tril",label="Check for three-liners (slowish, needs video res)",value=true},
	...
	buttons={">QC","Clear QC","Dial 5","Check all","Uncheck","Fuck this"}
	...
	pressed,res=aegisub.dialog.display(dialog_config,buttons,{ok='>QC',cancel='Fuck this'})
	...
	if pressed=="Fuck this" then aegisub.cancel() end
end
