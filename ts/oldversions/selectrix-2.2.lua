...
function slct(subs, sel)
	...
	if res.mode=="char. per second" then numb=cps end
	...
	if res.mode=="layer" or res.mode=="duration" or res.mode=="word count" or res.mode=="character count" or res.mode=="char. per second" then 
...
function konfig(subs, sel)
	...
	dialog_config=
	{
		...
		items={"--------text--------","style","actor","effect","text","visible text (no tags)","------numbers------","layer","duration","word count","character count","char. per second","------sorting only------","reverse","dialogue first","dialogue last","ts/dialogue/oped","{TS} to the top","by comments"}},
