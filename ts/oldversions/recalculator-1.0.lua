...
function calc(num)
    c=res.pc/100
    if pressed=="Multiply" then num=num*c end
...
local meta,styles=karaskel.collect_head(subs,false)
	...
	if res.bord and not text:match("\\bord") then text=text:gsub("^({\\[^}]*)}","%1\\bord"..brdr.."}") end
		shdw=line.styleref.shadow
	if res.shad and not text:match("\\shad") then text=text:gsub("^({\\[^}]*)}","%1\\shad"..shdw.."}") end
		spac=line.styleref.spacing
	if res.fsp and not text:match("\\fsp") then text=text:gsub("^({\\[^}]*)}","%1\\fsp"..spac.."}") end
...
function recalculator(subs, sel)
	...
	pressed, res = aegisub.dialog.display(dialog_config,
		{"Multiply","Add","Cancel"},{cancel='Cancel'})
