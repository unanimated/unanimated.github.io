...
function konfig(subs, sel)
	...
	dialog_config=
	{
		...
		items={"--------text--------","style","actor","effect","text","visible text (no tags)","------numbers------","layer","duration","word count","character count","char. per second","blur","------sorting only------","sort by time","reverse","dialogue first","dialogue last","ts/dialogue/oped","{TS} to the top","masks to the bottom","by comments"}},
	    ...
	    items={"Default style - All","nonDefault - All","OP in style","ED in style","layer 0","skiddiks, your their?","----from selection----","no-blur signs","commented lines"}},
	...
	if pressed=="Preset" then 
		if res.pres=="no-blur signs" or res.pres=="commented lines" then presel(subs, sel)
...
function sorting(subs,sel)
    ...
    if res.mode=="sort by time" then table.sort(subtable,function(a,b) return a.start_time<b.start_time or (a.start_time==b.start_time and a.i<b.i) end) end
