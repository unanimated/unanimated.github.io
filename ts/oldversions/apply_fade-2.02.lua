...
function fade(subs, sel)
	...
	local text=subs[i].text
		...
		--aegisub.log("fadein: "..fadein.."   lf: "..lf.."   length: "..length.."   ftime1: "..ftime1)
		    ...
		    for text2 in text:gmatch("([%w%p][%s%*]?%s?%*?)") do
		    fin1=ftime1*count
		    fin2=fin1+lf
...
function fadeconfig(subs, sel)
	...
	dialog_config=
	{
	    ...
	    {x=0,y=5,width=2,height=1,class="label",label="Letter by letter:"},
	    {x=2,y=5,width=1,height=1,class="dropdown",name="letterfade",
		items={"40","80","120","160","200","250","300","350","400","450","500","1000"},value=lastlbl},
	    {x=3,y=5,width=2,height=1,class="label",label="ms for each letter", },
