...
italics=0	lbreak=0	hororifix=0	zeroes=0	badita=0	dotdot=0	comfail=0	dword=0		contr=0
report=""
  ...
  if res["clear"]==false and res["clearqc"]==false then
	...
	if prevline.class=="dialogue" and line.comment==false and start<prevstart then
		effect=effect.." ...sort by time pls"
		sorted=1
	end	end
	...
	visible2w=visible.."."
		...
		if a==b then effect=effect.." [double word]" dword=dword+1 end
		end
	...
	if visible:match("[{}]") then comfail=comfail+1 effect=effect.." [comment fail]" end
	...
	if line.comment==false and visible:match("[%.%,]\"") then inside=inside+1 end
	if line.comment==false and visible:match("\"[%.%,][^%.]") then outside=outside+1 end
	
	-- Hdr request against jdpsetting
	if text:match("{\\an8\\bord[%d%.]+\\pos%([%d%.%,]*%)}") then actor=" What are you doing..." end
	
	if text:match("embarass") then effect="how embarrassing" end
	
	-- retarded / pointless contractions
	if visible:match("there're") or visible:match("this's") then contr=contr+1 effect=effect.." [pointless contractions]" end
	...
	if res["lbreax"] and def==1 then
		if visible:match("\\N") then lbreak=lbreak+1 end
	end
	...
	if res["honorifix"] and def==1 and line.comment==false then
		...
		or visible:match("%a%-sama[^%a]") or visible:match("%a%-senpai")
		or visible:match("%a%-sensei") then hororifix=hororifix+1 actor=actor.."honorofix" end
	end
