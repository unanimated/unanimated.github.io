...
script_version = "1.4"		-- Split by \N: allow splitting lines by spaces if there's no \N; fixed so it doesn't split by spaces in comments
...
function konfig(subs, sel)
	...
	pressed, res = aegisub.dialog.display(dialog_config,{"fax it","multimove","bottom alpha","copy tags","split by \\N","cancel"})
	if pressed=="cancel" then aegisub.cancel() end
