...
italics=0	lbreak=0	hororifix=0	zeroes=0	badita=0
report=""
  ...
  if results["clear"]==false and results["clearqc"]==false then
	...
	itafail=0
	for it1, it2 in text:gmatch("\\i([01]).-\\i([01])") do
	if it1==it2 then itafail=1 end		end
	for it3, it4, it5 in text:gmatch("\\i([01]).-\\i([01]).-\\i([01])") do
	if it3==it4 or it4==it5 then itafail=1 end		end
    ...
    if sorted==0 and mblur==0 and malf==0 and dis==0 and over==0 and gap==0 and dspace==0 then
    ...
    pressed,res=aegisub.dialog.display(reportdialog,{"OK","Copy to clipboard"})
    if pressed=="Copy to clipboard" then    clipboard.set(report) end
  end
end
