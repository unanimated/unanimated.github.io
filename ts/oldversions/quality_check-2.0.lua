...
report=""	styles=""	misstyles=""	fontlist=""	fontable={}
  ...
  if pressed==">QC" then
	...
	ll=linelen ra=0
	if res.read and def==1 and dura>50 and alfatime==0 then			-- these could use rephrasing if possible
	...
	if res.noread and def==1 and dura>50 and alfatime==0 then			-- from here on, it's bad. rephrase/retime
	...
	-- lines under 0.5s
	if res.halfsec and def==1 and visible~="" and ll>8 then
    ...
    if sorted==0 and mblur==0 and malf==0 and dis==0 and over==0 and gap==0 and dspace==0 and dotdot==0 and badita==0 and comfail==0 and unreadable==0 and misstyles=="" then
    ...
    pressd,rez=aegisub.dialog.display(reportdialog,{"OK","Copy to clipboard","Cancel"},{ok='OK',cancel='Cancel'})
...
function heather(subs)
    ...
    for i=1, #subs do
	    ...
	    if colorspace=="TV.601" then colorspace=colorspace.." (*Expiry Date: 2013-12-31*)" end
	end
    end
end
