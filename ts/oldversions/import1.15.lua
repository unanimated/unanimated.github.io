...
script_version="1.15"
	...
	IMPORT SIGN / IMPORT SIGNS - works like OP/ED, but you have to input the sign's name.
	    ...
	    SIGN - each sign must be saved in its own .ass file. In the GUI, input the sign's/file's name, for example "eptitle"[.ass].
	    SIGNS - all signs must be saved in signs.ass. They are distinguished by what's in the "effect" field - that's the sign's name.
	...
	Options:
	   ...
	   "delete original line" - this overrides the "keep line" option in the main menu. (I thought it would be convenient to have it here.)
	You can use relative or absolute paths. (Check the settings below.)
	Default is the script's folder. If you want the default one folder up, use "..\\".
	You can use an absolute path, have one huge signs.ass there, and have all the signs marked "show_name-sign_name" in the effect field.
	...
	"framerate" converts framerate from a to b where a is the input from "Left" and b is input from "Right".
...
function important(subs, sel, act)
	...
	if res.mega:match("import") then
		...
		{x=0,y=2,width=1,height=1,class="checkbox",name="matchtime",label="match times to current line",value=false,},
	    ...
	    if res.mega:match("sign") and reslt.noshift then shiftime=0 end
		...
		if res.mega:match("import sign") and reslt.matchtime then l2.start_time=basetime l2.end_time=basend else
		  ...
		  if not reslt.noshift then s_time=s_time+basetime  e_time=e_time+basetime end
		...
		l2.margin_t=margv
		l2.effect=eff
		if reslt.defect then l2.effect="" end
		l2.text=txt
		atext=txt 
	    ...
	    if reslt.deline then res.keep=false end
...
function numbers(subs, sel)
	...
	if res.modzero=="framerate" then
	    line.start_time=line.start_time/sub2*sub1
	    line.end_time=line.end_time/sub2*sub1
	end
...
if datata==nil then data="" else data=datata end
	...
	{x=8,y=11,width=1,height=1,class="dropdown",name="sav",items={"script","video"},value=default_save_name},
	    {x=6,y=12,width=3,height=1,class="edit",name="lang"},
	    ...
	    {x=6,y=14,width=2,height=1,class="dropdown",name="modzero",items={"number lines","add marker","framerate"},value="number lines"},
