...
function modifier(subs, sel)
	...
	local text=line.text
		...
		-- lots of retarded code here, apparently useless, but i'll leave it for now just in case
		-- top line length
		if c2[1].x>c2[2].x then topx=c2[1].x-c2[2].x else topx=c2[2].x-c2[1].x end
		if c2[1].y>c2[2].y then topy=c2[1].y-c2[2].y else topy=c2[2].y-c2[1].y end
		--top=math.sqrt(topx^2+topy^2)
		-- bottom line length
		if c2[3].x>c2[4].x then botx=c2[3].x-c2[4].x else botx=c2[4].x-c2[3].x end
		if c2[3].y>c2[4].y then boty=c2[3].y-c2[4].y else boty=c2[4].y-c2[3].y end
		--bot=math.sqrt(botx^2+boty^2)
		-- left line length
		if c[1].x>c[2].x then leftx=c[1].x-c[2].x else leftx=c[2].x-c[1].x end
		if c[1].y>c[2].y then lefty=c[1].y-c[2].y else lefty=c[2].y-c[1].y end
		--left=math.sqrt(leftx^2+lefty^2)
		-- right line length
		if c[3].x>c[4].x then ritex=c[3].x-c[4].x else ritex=c[4].x-c[3].x end
		if c[3].y>c[4].y then ritey=c[3].y-c[4].y else ritey=c[4].y-c[3].y end
		--rite=math.sqrt(ritex^2+ritey^2)
...
function relocator(subs, sel)
	dialog_config=
	{
		...
		items={"round numbers","line2fbf","join fbf lines","killmovetimes","fullmovetimes","fulltranstimes","set origin","calculate origin","FReeZe","rotate 180","flip hor.","flip vert.","negative rot","letterbreak","wordbreak"},value="round numbers"},
