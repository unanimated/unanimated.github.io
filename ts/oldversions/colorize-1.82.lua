...
if res.mode=="adjust RGB" then
	...
	styleref=stylechk(subs,line.style)
		...
		primary=styleref.color1:gsub("H%x%x","H")	sc1=primary
		pri=text:match("^{[^}]-\\c(&H%x+&)")
		if pri~=nil then primary=pri end
		
		outline=styleref.color3:gsub("H%x%x","H")	sc3=outline
		out=text:match("^{[^}]-\\3c(&H%x+&)")
		if out~=nil then outline=out end
		
		shadow=styleref.color4:gsub("H%x%x","H")	sc4=shadow
		sha=text:match("^{[^}]-\\c(&H%x+&)")
		if sha~=nil then shadow=sha end
		
		secondary=styleref.color2:gsub("H%x%x","H")	sc2=secondary
