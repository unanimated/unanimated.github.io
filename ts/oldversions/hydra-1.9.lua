...
function hydralast(subs, sel)
    if hydrazine==1 then
    hh9(subs, sel)
    else aegisub.dialog.display({{class="label",label="No saved settings.",x=0,y=0,width=1,height=2}},{"OK"},{cancel='OK'}) end
    aegisub.set_undo_point(script_name)
    return sel
end

aegisub.register_macro(script_name, script_description, hydra)
aegisub.register_macro("HYDRA - repeat with last settings", script_description, hydralast)