...
starts_after=0			-- same
ends_after=0			-- same
use_preset=false		-- use preset for keyframes [true/false]
...
function keyframesnap(subs, sel)
	for z, i in ipairs(sel) do
	    ...
	    --aegisub.log("kfsb "..kfsb.."\n")
	    --aegisub.log("kfeb "..kfeb.."\n")
	    --aegisub.log("kfsa "..kfsa.."\n")
	    --aegisub.log("kfea "..kfea.."\n")
...
function konfig(subs, sel)
	dialog_config=
	{
	    {	x=0,y=0,width=5,height=1,class="label",label="Applies to selected lines. Times below are in milliseconds.", },
	...
	pressed, res = aegisub.dialog.display(dialog_config,
		{"[ cut or add lead in ]","[ cut or add lead out ]","shift times","line linking","kf snapping","cancel"})
