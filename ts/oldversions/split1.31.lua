...
function split(subs, sel)
	for i=#sel,1,-1 do
	  ...
	  text=subs[sel[i]].text
	    if not text:match("\\N") and text:match("%- ") then text=text:gsub("%- (.-)$","\\N%1") end
		...
		if text:match("that$") then text=text:gsub(" that$","") nextline.text="that "..nextline.text end
		if text:match("and$") then text=text:gsub(" and$","") nextline.text="and "..nextline.text end
		if text:match("but$") then text=text:gsub(" but$","") nextline.text="but "..nextline.text end
		if text:match("so$") then text=text:gsub(" so$","") nextline.text="so "..nextline.text end
		if text:match("to$") then text=text:gsub(" to$","") nextline.text="to "..nextline.text end
		if text:match("with$") then text=text:gsub(" with$","") nextline.text="with "..nextline.text end
		if text:match("the$") then text=text:gsub(" the$","") nextline.text="the "..nextline.text end
