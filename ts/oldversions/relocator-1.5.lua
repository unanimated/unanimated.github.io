...
function clone(subs, sel)
	...
	if res.clip then
		...
		if klip2:match("m") then type2="vector" else type2="normal" end
		if text:match("\\(i?)clip.-\\(i?)clip") then doubleclip=true else doubleclip=false end
		-- 1 clip, stack
		if res.stack and type1~=type2 and not doubleclip then 
		  text=text:gsub("^({\\[^}]*)}","%1\\"..ik.."clip%("..klip.."%)}")
		-- 2 clips -> not stack
		elseif doubleclip then
		  if type1=="normal" then text=text:gsub("\\(i?clip)%([%d%.,%-]-%)","\\%1%("..klip.."%)") end
		  if type1=="vector" then text=text:gsub("\\(i?clip)%(%d?,?m[^%)]-%)","\\%1%("..klip.."%)") end
		-- 1 clip, not stack
		elseif type1==type2 and not doubleclip or not res.stack and not doubleclip then
		  text=text:gsub("\\i?clip%([^%)]-%)","\\"..kmatch.."clip%("..klip.."%)")
		end
	    end
