...
prevent_cut_overlaps=true	-- prevent overlaps from adding lead in/out
...
function cutout(subs, sel)
	for z, i in ipairs(sel) do
		...
		if res.stail=="Default" and line.style:match("Defa") then run=1 end
		...
		-- add
		endt=(endt+ut)		if res.preventcut and i<#subs and endt>nextart then endt=nextart end
		end
	    line.end_time=endt
	    end
...
function cutin(subs, sel)
	for z, i in ipairs(sel) do
		...
		-- add
		start=(start-inn)	if res.preventcut and prevline.class=="dialogue" and start<prevend then start=prevend end
		end
...
function linklines(subs, sel)
	...
	for z, i in ipairs(sel) do
		...
		run=0
		if res.stail=="All" then run=1 end
		if res.stail=="Default" and line.style:match("Defa") and nextline.style:match("Defa") then run=1 end
		if res.stail=="Default+Alt" and line.style:match("Defa") and nextline.style:match("Defa") then run=1 end
		if res.stail=="Default+Alt" and line.style:match("Alt") and nextline.style:match("Alt") then run=1 end
		if res.stail==line.style and res.stail==nextline.style then run=1 end
...
function konfig(subs, sel)
	...
	dialog_config=
	{
	    ...
	    {x=2,y=0,width=2,height=1,class="dropdown",name="slct",
	    ...
	    {x=2,y=1,width=2,height=1,class="dropdown",name="stail",
	    items=styles,value=styles_to_apply_to},
	    ...
	    {x=8,y=5,width=1,height=1,class="dropdown",name="preset",
