...
-- NOTES:
-- "back and forth transform" uses the "interval" field
-- "split line in 3 parts" uses t1 and t2 fields
-- "create 3D effect from shadow" requires \xshad \yshad
...
function hh9(subs, sel)
	...
	if not text:match("^{\\") then text="{\\tag}"..text end		-- add {\} if line has no tags
	
	-- tag position
	place=res.linetext
	if place:match("*") then pl1,pl2,pl3=place:match("(.*)(%*)(.*)") pla=1 else pla=0 end
	if res.tagpres~="--- presets ---" then pla=1 end
...
linetext=oneline.text:gsub("{[^}]-}","")
	dialog_config=
	{
	    ...
	    {x=3,y=6,width=1,height=1,class="floatedit",name="be2",value=1,min=0 },
