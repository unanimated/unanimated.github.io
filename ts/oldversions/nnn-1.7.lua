...
min_characters=42		-- minimum characters needed to insert a linebreak (ie no breaks in lines under X characters)	[42]
...
middle_min_characters=45	-- minimum of characters needed to insert a linebreak in the middle				[45]
...
enable_shift_backwards=false	-- enables shifting backwards (adds a new item to the automation menu)				[false]
disable_dialog=false		-- if no break is placed with the insert function, a dialog lets you choose it manually.	[false]
allow_break_between_two=false	-- allow a break if there are only 2 words. good for TS, not much for editing (overrides min_ settings)	[false]
...
function nnn(subs, sel)
	...
	--aegisub.log("text - "..tekst)
	    ...
	    -- remove breaks if there are more than one; leave the one closes to the centre, round 1
	    ...
	    if do_balance_checks and tekst:match("\\N") and wrd>4 then
