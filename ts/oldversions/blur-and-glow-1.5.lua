...
automatically_use_double_border=false		-- true/false; automatically use double border if 2nd colour or 2nd border size is checked
...
--		line.effect="done"
	...
	line2.text = line2.text:gsub("^({\\[^}]-)}","%1\\t(" .. line.duration-fadout .."," .. line.duration-fadout+res.alphade .. ",\\1a&HFF&)}")
	    ...
	    if res.alfa=="00" then text=text:gsub("\\alpha&H00&","") end
...
function konfig(subs, sel)
	...
	if pressed=="Blur + Glow" then glow(subs, sel) end
