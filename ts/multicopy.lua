...
function copyc(subs, sel)	-- clip etc
	...
	local text=subs[i].text
	      ...
	      if x~=#sel and res.copymode=="actor" then
	      copyclip=copyclip..line.actor.."\n"
	      end
	      if x==#sel and res.copymode=="actor" then
	      copyclip=copyclip..line.actor
	      end
	      
	      if x~=#sel and res.copymode=="effect" then
	      copyclip=copyclip..line.effect.."\n"
	      end
	      if x==#sel and res.copymode=="effect" then
	      copyclip=copyclip..line.effect
	      end
...
raw=res.dat	raw=raw:gsub("\n","")
  ...
  if res.oneline==true then
	...
	if res.pastemode=="all" then text=raw end
	
	if res.pastemode=="layer" then line.layer=raw end
	
	if res.pastemode=="actor" then line.actor=raw end
	
	if res.pastemode=="effect" then line.effect=raw end
	
	if res.pastemode=="duration" then line.end_time=line.start_time+raw end

	if res.pastemode=="raw" then line.raw=raw end
	...
	if res.pastemode=="actor" then line.actor=data[x] end
	
	if res.pastemode=="effect" then line.effect=data[x] end
	
	if res.pastemode=="duration" then line.end_time=line.start_time+data[x] end
	
	-- if res.pastemode=="raw" then line.raw=data[x] end aegisub.log("\nraw "..line.raw) aegisub.log("\ntext "..line.text)
...
function multicopy(subs, sel)
	...
	if pressed=="Copy" then    
	    ...
	    or res.copymode=="layer" or res.copymode=="duration" or res.copymode=="raw" or res.copymode=="actor" or res.copymode=="effect" 
	    then copyc(subs, sel) end	
