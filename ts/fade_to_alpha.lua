...
function fadalpha(subs, sel)
	...
	-- with alpha in line
		...
		-- fade to colour
			    if res.clr then
			text=text:gsub("^({\\[^}]-)}","%1\\t("..dur-fadout..",0,"..res.ut..","..kolorb..")}")
		-- inline colour tags
			for t in text:gmatch("({\\[^}]-})") do
				if t~=text:match("^{\\[^}]-}") and t:match("\\[13]?c") then
		...
		-- fade to alpha
			...
			text=text:gsub("^({\\[^}]-)}","%1\\t("..dur-fadout..",0,"..res.ut..","..aff..")}")
		-- inline alpha tags
			for t in text:gmatch("({\\[^}]-})") do
				if t~=text:match("^{\\[^}]-}") and t:match("\\alpha") then
...
function konfig(subs, sel)
	dialog_config=
	{
	    {x=0,y=0,width=3,height=1,class="label",label="This will turn \\fad into \\t\\alpha",},
	    {x=0,y=1,width=1,height=1,class="label",label="accel in:",},
	    {x=0,y=2,width=1,height=1,class="label",label="accel out:",},
	    {x=1,y=1,width=2,height=1,class="floatedit",name="inn",value=1},
	    {x=1,y=2,width=2,height=1,class="floatedit",name="ut",value=1},
	    {x=0,y=3,width=3,height=1,class="label",label="  <1 starts fast, ends slow \n  >1 starts slow, ends fast",},
	    {x=0,y=4,width=2,height=1,class="checkbox",name="crl",label="fade from:    ",value=false},
	    {x=0,y=5,width=2,height=1,class="checkbox",name="clr",label="fade to:",value=false},
	    {x=2,y=4,width=1,height=1,class="color",name="c1"},
	    {x=2,y=5,width=1,height=1,class="color",name="c2"},
	} 	
	pressed, res=aegisub.dialog.display(dialog_config,{"Transform","Cancel"},{ok='Transform',close='Cancel'})
